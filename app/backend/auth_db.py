...
def seed_demo_brand_sets() -> None:
    ...
    Each snapshot was generated by `scripts/seed_demo_brand.py` against a real
    investor deck (Snowflake PPTX, Caterpillar / Salesforce / BlackRock PDFs).
    We commit them so every deploy ships the showcase decks without re-mining.

    Snapshot id pattern: `tpl_demo_<slug>` (deterministic — the same slug
    always maps to the same row). Lives under the `__percy_system__` sentinel
    org alongside Percy Standard; visible to every user via is_builtin=1.
