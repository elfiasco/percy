...
import { fetchBuzzwordDensity } from "../../lib/studioApi"
import type { BuzzwordSlide } from "../../lib/studioApi"
...
const labelColor = (l: BuzzwordSlide["label"]) =>
...
export default function BuzzwordDensityModal({ docId, onClose, onJumpToSlide }: Props) {
  const [loading, setLoading] = useState(true)
  const [data, setData]       = useState<{ slides: BuzzwordSlide[]; total_buzzwords: number } | null>(null)
  ...
  const run = async () => {
    ...
    setError("")
    try {
      setData(await fetchBuzzwordDensity(docId))
    } catch {
      setError("Failed to analyze buzzword density")
    } finally {
      setLoading(false)
    }
  }
    ...
    >
      <div className="bg-[#1e1e2e] border border-white/10 rounded-xl shadow-2xl w-[560px] max-h-[88vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
          <div>
            <h2 className="text-white font-semibold text-sm">Buzzword Density</h2>
            <p className="text-white/40 text-xs mt-0.5">Flag slides overloaded with corporate buzzwords</p>
          </div>
          ...
          {data && !loading && (
            <>
              <div className="flex items-center gap-4 text-xs text-white/40">
                <span>Total: <span className="text-white/70">{data.total_buzzwords}</span></span>
                ...
                {slides.map((s) => (
                  <div key={s.slide_n} className="bg-white/3 border border-white/8 rounded-lg px-3 py-2.5 space-y-1.5">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="text-white/30 text-[10px]">{s.count} word{s.count !== 1 ? "s" : ""}</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded border ${labelColor(s.label)}`}>{s.label}</span>
                      </div>
                    </div>
                    {s.buzzwords.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {s.buzzwords.map(bw => (
                          <span key={bw} className="text-[10px] px-1.5 py-0.5 rounded border border-orange-400/20 bg-orange-400/8 text-orange-300">{bw}</span>
                        ))}
                      </div>
                    )}
                  </div>
