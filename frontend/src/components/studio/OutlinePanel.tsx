...
export default function OutlinePanel({ docId, slideCount, selectedSlide, refreshKey, onJumpToSlide }: Props) {
  ...
  const filtered = search.trim()
    ? entries.filter((e) =>
        ...
        (e.body_preview ?? "").toLowerCase().includes(search.toLowerCase()) ||
        (slideLabels[e.slide_n] ?? "").toLowerCase().includes(search.toLowerCase())
      )
    : entries
    ...
    <div className="w-52 shrink-0 border-r border-edge bg-surface flex flex-col overflow-hidden">
      ...
      <div className="px-2 py-1.5 border-b border-edge/40 shrink-0">
        <input
          ...
          className="w-full text-[10px] bg-base border border-edge rounded px-2 py-1 text-slate-300
